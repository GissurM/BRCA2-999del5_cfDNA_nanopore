#!/usr/bin/env python3
"""
Genome-wide coding-region methylation PERMANOVA/PERMDISP for BRCA2/control cfDNA.
...
Pipeline
1) Build merged per-gene coding regions from GENCODE CDS feature spans.
2) Aggregate methylated/total CpG counts per coding region per sample via bedtools intersect.
3) Build sample x coding-region methylation matrix (percent methylation).
4) Filter coding regions by per-group minimum observed samples, median-impute remaining sparse NAs.
...
Notes
- Coding-region coordinates are taken from GENCODE CDS feature records.
- CDS intervals for each gene are merged into non-overlapping coding regions.
- Regions are labeled GeneName-1, GeneName-2, ... in transcriptional order.
...
DEFAULT_OUTPUT_DIR = "/mnt/d/BRCA2-misc-files/mod_BED_coronary/permanova_genebody_output"
...
class GencodeCodingRegionCatalog:
    """Build merged coding-region catalog from GENCODE CDS feature records."""
    ...
    def build_catalog(self) -> List[Dict[str, object]]:
        ...
        with opener(self.gtf_path, "rt") as handle:
            for line in handle:
                ...
                if gene_name not in cds_by_gene:
                    cds_by_gene[gene_name] = {
                        "gene_name": gene_name,
                        "gene_id": attr_map.get("gene_id", "N/A"),
                        "chromosome": chrom_name,
                        "strand": strand,
                        "cds_intervals": [(start_i, end_i)],
                    }
                else:
                    cds_by_gene[gene_name]["cds_intervals"].append((start_i, end_i))

        regions: List[Dict[str, object]] = []
        for gene_name in sorted(cds_by_gene.keys()):
            info = cds_by_gene[gene_name]
            strand = str(info["strand"])
            intervals = sorted(set(info["cds_intervals"]), key=lambda x: (x[0], x[1]))
            if not intervals:
                continue
            ...
            ordered = merged if strand != "-" else list(reversed(merged))
            for region_idx, (start_i, end_i) in enumerate(ordered, start=1):
                regions.append(
                    {
                        "region_id": f"{gene_name}-{region_idx}",
                        "gene_name": gene_name,
                        "region_index": region_idx,
                        "gene_id": info.get("gene_id", "N/A"),
                        "chromosome": info["chromosome"],
                        "strand": strand,
                        "region_start": start_i,
                        "region_end": end_i,
                    }
                )
...
def run_metric_outputs(
    ...
    qc_df.to_csv(outdir / f"coding_region_matrix_qc_{metric_key}.csv", index=False)
...
def plot_spearman_pair(
    ...
    plt.xlabel(
        f"{label_a} weighted mean coding-region {metric_label} (%)",
        fontsize=15,
        fontweight="bold",
    )
    plt.ylabel(
        f"{label_b} weighted mean coding-region {metric_label} (%)",
        fontsize=15,
        fontweight="bold",
    )
    plt.title(
        f"Coding-region {metric_label} concordance: {label_a} vs {label_b}\n"
...
def run_analysis(args: argparse.Namespace) -> None:
    ...
    region_items = GencodeCodingRegionCatalog(args.gencode_gtf).build_catalog()
    region_count = len(region_items)
    print(f"Loaded coding regions: {region_count:,}")
    ...
    if len(kept_regions) < 20:
        raise RuntimeError(
            "Too few coding regions left after filtering. Try lowering --min-cpgs or --min-samples."
        )
    ...
    qc_df.to_csv(outdir / "coding_region_matrix_qc.csv", index=False)

    lines = [
        "# Genome-wide coding-region methylation PERMANOVA/PERMDISP",
        "",
        "## Inputs",
        ...
        f"- Coding regions total: {region_count:,}",
        f"- Coding regions kept: {len(kept_regions):,}",
        f"- Significant coding regions (union of 5mC/5hmC, q<0.05): {int(sig_union.size):,}",
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Genome-wide coding-region methylation PERMANOVA/PERMDISP for BRCA2/control cfDNA"
    )
