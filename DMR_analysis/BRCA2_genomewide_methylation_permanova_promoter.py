...
DEFAULT_OUTPUT_DIR = "/mnt/d/BRCA2-misc-files/mod_BED_coronary/permanova_genomewide_output"
...
DEFAULT_GROUP_ORDER = ["brca2", "control", "recovery", "stemi"]
...
def aggregate_sample_with_bedtools(task: Tuple[str, str, int]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Aggregate per-promoter 5mC/5hmC and total CpG counts for one sample using bedtools."""
    ...
    for line in proc.stdout:
        ...
        if 0 <= gene_idx < gene_count:
            # With modkit without --combine-mods, m and h are separate rows per position.
            # Use Nmod from the matching row to avoid mirrored 5mC/5hmC artifacts.
            if mod_code == "m":
...
def run_metric_outputs(
    ...
    metric_imputed, na_fraction = median_impute_by_promoter(metric_pct)
    ...
    qc_df.to_csv(outdir / f"promoter_matrix_qc_{metric_key}.csv", index=False)

    return permanova_df, permdisp_df, sample_metric_df, metric_imputed


def gene_level_kruskal_tests(
    metric_pct: np.ndarray,
    groups: Sequence[str],
    group_order: Sequence[str],
    ...
    for j in range(metric_pct.shape[1]):
        ...
        for g in group_order:
            ...
            arrays.append(vals)
        if not valid_gene:
            rows.append({"gene": gene_names[j], "gene_index": j, "statistic": np.nan, "p_value": np.nan})
            continue
        try:
            stat, p_value = kruskal(*arrays)
        except ValueError:
            stat, p_value = np.nan, np.nan
        rows.append({"gene": gene_names[j], "gene_index": j, "statistic": float(stat), "p_value": float(p_value)})
...
def plot_significant_gene_stacked_composition(
    ...
    if significant_idx.size == 0:
        pd.DataFrame(
            columns=["group", "label", "pct_5mc", "pct_5hmc", "pct_unmodified", "n_significant_genes"]
        ...
        rows.append(
            {
                ...
                "n_significant_genes": int(significant_idx.size),
            }
        )
    ...
    plt.title(
        "Significant-gene CpG composition by group\n"
        f"(genes significant in >=1 group comparison for 5mC or 5hmC; n={int(significant_idx.size)})"
    )
    plt.ylim(0, 100)
...
def plot_sample_scatter(
    ...
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.title(f"Genome-wide promoter {metric_label} sample embedding")
    plt.grid(alpha=0.2)
    plt.legend(frameon=False)
    plt.tight_layout()
    plt.savefig(out_png, dpi=300, bbox_inches="tight")
...
def plot_global_boxplot(
    ...
    plt.ylabel(f"Coverage-weighted global promoter {metric_label} (%)")
    plt.title(f"Global {metric_label} by cohort")
...
def plot_spearman_pair(
    ...
    plt.xlabel(
        f"{label_a} weighted mean promoter {metric_label} (%)",
        fontsize=15,
        fontweight="bold",
    )
    plt.ylabel(
        f"{label_b} weighted mean promoter {metric_label} (%)",
        fontsize=15,
        fontweight="bold",
    )
    plt.title(
        f"Promoter {metric_label} concordance: {label_a} vs {label_b}\n"
...
def run_analysis(args: argparse.Namespace) -> None:
    ...
    kept_gene_names = [str(promoter_items[i][0]) for i in kept_genes]
    ...
    sample_out.rename(
        columns={
            "global_5mc_pct": "global_5mc_pct",
            "global_5hmc_pct": "global_5hmc_pct",
        },
        inplace=True,
    )
    sample_out.to_csv(outdir / "sample_global_5mc_5hmc.csv", index=False)

    gene_tests_5mc = gene_level_kruskal_tests(
        metric_pct=meth_5mc_pct,
        groups=groups,
        group_order=group_order,
        gene_names=kept_gene_names,
    )
    gene_tests_5mc.to_csv(outdir / "gene_level_5mc_group_kruskal.csv", index=False)
    export_significant_gene_table(
        test_df=gene_tests_5mc,
        out_html=outdir / "gene_level_5mc_group_kruskal_significant_genes.html",
        out_png=outdir / "gene_level_5mc_group_kruskal_significant_genes.png",
        title="Significant genes from Kruskal-Wallis tests (5mC)",
        q_threshold=0.05,
    )

    gene_tests_5hmc = gene_level_kruskal_tests(
        metric_pct=meth_5hmc_pct,
        groups=groups,
        group_order=group_order,
        gene_names=kept_gene_names,
    )
    gene_tests_5hmc.to_csv(outdir / "gene_level_5hmc_group_kruskal.csv", index=False)
    export_significant_gene_table(
        test_df=gene_tests_5hmc,
        out_html=outdir / "gene_level_5hmc_group_kruskal_significant_genes.html",
        out_png=outdir / "gene_level_5hmc_group_kruskal_significant_genes.png",
        title="Significant genes from Kruskal-Wallis tests (5hmC)",
        q_threshold=0.05,
    )

    sig_5mc = set(gene_tests_5mc.loc[gene_tests_5mc["q_value_bh"] < 0.05, "gene_index"].dropna().astype(int).tolist())
    sig_5hmc = set(gene_tests_5hmc.loc[gene_tests_5hmc["q_value_bh"] < 0.05, "gene_index"].dropna().astype(int).tolist())
    ...
    plot_significant_gene_stacked_composition(
        ...
        out_png=outdir / "stacked_sig_gene_cpg_composition_5mc_5hmc.png",
        out_csv=outdir / "stacked_sig_gene_cpg_composition_5mc_5hmc.csv",
    )
    ...
    qc_df = pd.DataFrame(
        {
            ...
            "n_sig_genes_union_5mc_5hmc": [int(sig_union.size)],
        }
    )
    ...
    lines = [
        ...
        f"- Significant genes (union of 5mC/5hmC, q<0.05): {int(sig_union.size):,}",
        "",
        "## Key outputs",
        ...
        "- gene_level_5mc_group_kruskal.csv",
        "- gene_level_5hmc_group_kruskal.csv",
        "- gene_level_5mc_group_kruskal_significant_genes.html",
        "- gene_level_5mc_group_kruskal_significant_genes.png",
        "- gene_level_5hmc_group_kruskal_significant_genes.html",
        "- gene_level_5hmc_group_kruskal_significant_genes.png",
        "- stacked_sig_gene_cpg_composition_5mc_5hmc.csv",
        "- stacked_sig_gene_cpg_composition_5mc_5hmc.png",
